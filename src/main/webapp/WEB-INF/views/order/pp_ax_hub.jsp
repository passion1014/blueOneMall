    ...
    */
    ...
    else if ( req_tx.equals( "mod_escrow" ) )
    {
        ...
        if ( mod_type.equals("STE2") || mod_type.equals("STE4") )                                       // 상태변경 타입이 [즉시취소] 또는 [취소]인 계좌이체, 가상계좌의 경우
        {
            if ( vcnt_yn.equals("Y") )
            {
                c_PayPlus.mf_set_us( mod_data_set_no, "refund_account", request.getParameter( "refund_account" ) );  // 환불수취계좌번호
                c_PayPlus.mf_set_us( mod_data_set_no, "refund_nm",      request.getParameter( "refund_nm"      ) );  // 환불수취계좌주명
                c_PayPlus.mf_set_us( mod_data_set_no, "bank_code",      request.getParameter( "bank_code"      ) );  // 환불수취은행코드
            }
        }
    }
    /* = -------------------------------------------------------------------------- = */
